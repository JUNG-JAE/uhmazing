...
from utils.tools import adjust_learning_rate, vali, load_content
...
# ---------------- 기본 설정 ----------------
parser.add_argument('--task_name', type=str, default='long_term_forecast', help='태스크 이름: [long_term_forecast, short_term_forecast]')
...
# ---------------- 데이터 로더 ----------------
parser.add_argument('--data', type=str, default='ETTh1', help='데이터셋 종류(Dataset 클래스 선택에 사용)')
parser.add_argument('--root_path', type=str, default='./dataset/ETT-small/', help='데이터 루트 경로')
parser.add_argument('--data_path', type=str, default='ETTh1.csv', help='데이터 파일명')
...
parser.add_argument('--freq', type=str, default='h', help='시간 특성 인코딩 주기: [s,t,h,d,b,w,m] (h=시간 단위)')
...
parser.add_argument('--pred_len', type=int, default=96, help='예측 시퀀스 길이') # 모델이 몇스탭을 예측할지 결정

# ---------------- 모델 정의 ----------------
parser.add_argument('--enc_in', type=int, default=7, help='입력 변수 개수(ETTh1=7)')
...
parser.add_argument('--moving_avg', type=int, default=25, help='이동평균 윈도우')
...
parser.add_argument('--gpt2_size', type=str, default='small', choices=['small', 'medium', 'large', 'xl'], help='GPT2 백본 크기(llm_model=GPT2일 때): small(768)/medium(1024)/large(1280)/xl(1600)')
...
parser.add_argument('--train_epochs', type=int, default=1, help='학습 에폭 수')
...
parser.add_argument('--lradj', type=str, default='type1', help='학습률 스케줄 방식')
parser.add_argument('--pct_start', type=float, default=0.2, help='OneCycle 워밍업 비율')
