...
# =============================================================================
# 섹션 7: 클래스 가중치 계산 (실험 B와 동일 방식)
# =============================================================================
print("\n[섹션 7] 클래스 가중치 계산...")

def compute_sample_weights(y, n_classes):
    """역빈도 기반 샘플 가중치 (TFT의 CE class_weight와 동일 로직)"""
...
wgrade_sample_weights, wgrade_class_weights = compute_sample_weights(
    y_wgrade_train, NUM_CLASSES['WGRADE_CLASS']
)
quality_sample_weights, quality_class_weights = compute_sample_weights(
    y_quality_train, NUM_CLASSES['QUALITY_GRADE_CLASS']
)

print(f"  WGRADE class weights: {[f'{w:.4f}' for w in wgrade_class_weights]}")
print(f"  QUALITY_GRADE class weights: {[f'{w:.4f}' for w in quality_class_weights]}")
